...
def create_training_instances(input_file, output_file, max_seq_length,
    ...
    for _ in range(dupe_factor):
        for document_index in range(len(all_documents)):
            a = create_instances_from_document(
                ds, all_documents, [], document_index, max_seq_length, short_seq_prob,
    ...
    while i < document[0]:
        ...
        if i == document[0] - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                #    input_ids + input_mask + segment_ids + masked_lm_labels + [next_sentence_label]))
                a.append(torch.IntTensor(
                    input_ids + input_mask + segment_ids + masked_lm_labels + [next_sentence_label])
                )
            current_chunk = []
            current_length = 0
